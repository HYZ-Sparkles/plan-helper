...
impl AllocationService {
    ...
    /// 自动打开大面板的判定（AutoOpenMainBoard）：工作模式 + 今日未分配；
    /// 工作日条件只约束**自动触发**（启动序列后检测 / 13 的工作窗口开始）——manual =
    /// 手动切入工作模式（用户主动选加班，2026-08-29 决策）旁路它，休息日同样弹面板选任务。
    /// 日期例外叠加在工单 13。
    pub fn should_auto_open(
        ...
        Ok(work_mode
            && (manual || SettingsService::is_workday(conn, clock).map_err(db_err)?)
...
fn selectable_of(groups: &[AllocationGroup]) -> Vec<i64> {
    groups.iter().flat_map(|g| &g.tasks).map(|t| t.id).collect()
}
