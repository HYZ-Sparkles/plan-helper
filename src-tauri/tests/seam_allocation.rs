...
fn board_groups_active_plans_with_dependency_gating() {
    ...
    let tasks = &v.groups[1].tasks;
    assert_eq!(tasks.len(), 3, "未完成任务全部在场（含被阻塞的 B）");
    let b = tasks.iter().find(|t| t.name == "B").unwrap();
    assert_eq!(b.waiting_on, vec!["A".to_string()], "前置未完成 → 等待名单");
    let sg = tasks.iter().find(|t| t.name == "SG").unwrap();
    assert_eq!(sg.estimated_minutes, 90, "子目标任务耗时 = 子目标求和");

    force_task_status(&conn, tasks[0].id, TaskStatus::Completed);
    let v2 = AllocationService::board(&conn, &at(2026, 8, 24, 9, 0)).unwrap();
    let mid = &v2.groups[1].tasks;
    assert_eq!(mid.len(), 2, "已完成任务不进候选");
    let b2 = mid.iter().find(|t| t.name == "B").unwrap();
    assert_eq!(b2.waiting_on, Vec::<String>::new(), "A 完成后 B 实时解锁");
}
...
fn commit_persists_overwrites_and_board_roundtrips() {
    ...
    // B 前置 A 未完成：直接提交被服务层拒绝，之前的分配保持不变
    assert_eq!(
        AllocationService::commit(&conn, &at(2026, 8, 24, 16, 0), &[_b]),
        Err(PlanError::TaskNotAllocatable { task_id: _b })
    );
...
fn commit_rejects_non_selectable_tasks() {
    // 测试情况：分别提交 前置未完成的任务 / 已完成任务 / 未开始计划里的任务 / 不存在 id。
    ...
    let v = AllocationService::board(&conn, &clock).unwrap();
    let active_ids: Vec<i64> = v.groups[0].tasks.iter().map(|t| t.id).collect();
    // SG 置为已完成（A 保持未完成，B 的阻塞才有意义）
    force_task_status(&conn, active_ids[2], TaskStatus::Completed);

    let idle_id: i64 = conn
        .query_row(
            "SELECT t.id FROM tasks t JOIN plans pl ON pl.id = t.plan_id WHERE pl.id = ?1 AND t.name = 'A'",
            rusqlite::params![p_idle],
            |r| r.get(0),
        )
        .unwrap();
    for bad in [active_ids[1] /* B 被阻塞 */, active_ids[2] /* 已完成 */, idle_id, 999] {
        assert_eq!(
