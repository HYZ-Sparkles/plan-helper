...
**2026-08-28 补充二：帧序覆写 + 每帧位移权重（用户验收沟通需要）**

- **素材排帧顺序 ≠ 播放顺序**：生成脚本 `FRAME_ORDER` 表重排（动画号 → 播放顺序，元素 = 素材从左数第几帧 1-based；切分矩形不动、矩形跟着帧走）。NUDGE/MOVE_WEIGHTS 的帧号一律指**重排后的播放位**；两张新表长度与切分帧数不符时脚本报错拒绝生成（防手误）。
- **运动帧行程不均**：`AnimationDef.moveWeights`（每帧行程占比 = w[i]/Σw，缺省全 1 = 均匀现状）。引擎 `moveProgress`：未配权重走原时间线性路径（零回归），配了沿累积权重曲线分段线性采样；末步终点钉死 p=1 不变——Run 跨步快/收腿慢、Jump 蓄力缓/腾空疾都靠权重表达，09 的 Run/Eat/Walk 配 movement 时直接受益。
- 调试页平铺格改两排控件：上排 ◁▷ 播放位移位 + −/＋ 权重步进（0.5 步、权重跟素材帧走），下排 ◀▲▼▶ 摆放微调（↺ 恢复本帧默认）；帧序/权重实时改写 def（引擎每 tick 取 def，播放中立即可见）；「恢复原序」「权重归一」「清空草稿」复原；页底草稿导出三表合一（FRAME_ORDER / MOVE_WEIGHTS / NUDGE，一键复制）。
- 验证：Node 确定性回归（权重曲线帧边界/中点/终点精确、线性路径不变、onSettle 恰一次；生成器重排发射/NUDGE 播放位语义/长度校验拒绝）+ vue-tsc + 浏览器实测（换序、权重读数 1.5、三表草稿、带权重位移播放终点精确 200px、恢复原序后草稿仅剩权重与微调、清空草稿导出块消失）+ 视觉核对换序后精灵图跟走（首格变宽猫、次格变窄猫）。

**2026-08-29 验收修复：启动后一直站立（看似进了休息模式）**

- 根因：启动序列落地后 700ms 转工作睡眠的守卫误判 `engine.state().busy`——稳态 idle 循环里 `action` 永不清空、`busy` 恒真，4 Stand to Sleep 从不触发，桌宠停在 7 Stand Idle（休息模式的常驻动作），与实际 mode=work 不符。改判 `locked`（锁只在过渡期持有，稳态循环即空闲），启动后 700ms 如期 4→5 进 Sleep Idle。
- 顺带接线：`checkAutoOpen(manual)`——启动序列后的自动检测传 false（非工作日不弹）；手动切入工作模式传 true（主动加班，休息日也弹大面板，面板加班态见 06 的验收修订）。

**2026-08-29 验收修订二（用户决策）：初始模式按工作时间判定**
