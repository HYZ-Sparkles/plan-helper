...
二次校准（2026-09-14，OpenAI codex-rs `pets/model.rs` 实证）：idle 帧时长改用应用实际 calm loop `[1680,660,660,840,840,1920]`（6.6s 一圈，参考表的 6 倍——参考表只对自制宠物包有效，此前的「太快」观感即源于此）；其余动作每帧时长与参考表一致无需改。
