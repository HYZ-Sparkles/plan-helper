...
**验收反馈修正（2026-08-30，第三轮：控制面板打开时底部超出屏幕）**

- 反馈：控制面板打开时总有一部分压在屏幕下沿外。
- 根因两层：① 大面板/总结窗都有 `"center": true`，唯独控制面板漏配——窗口落 OS 默认位置（压底摆放）；② 720 逻辑高度在高缩放屏物理放不下（150% 缩放的 1080p 屏逻辑桌面 1280×720、工作区 ≈688，怎么摆都超底）。
- 修法（采纳用户建议减高度 + 补齐定位）：height 720→640（150% 缩放下工作区上下各留 ~24px；100%/125% 更宽松），并补 `"center": true` 与另两窗对齐——只减高度不定位的话，OS 默认压底摆放仍可能超。minWidth/minHeight（880×600）不变，页面内容列自适应，无联动改动。
