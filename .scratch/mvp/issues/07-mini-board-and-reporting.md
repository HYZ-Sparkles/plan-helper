...
- [x] 无子目标任务：PercentAdjustControl（[-] -10% 钳制最低 0.1%、点数字直填任意正数〔最小 0.1%、最多一位小数〕、[+] +10% 钳制最高 100%，默认 5%），会话级不持久化（watch task_id 重置），切换任务重置（2026-08-24 验收修订：原"5% 倍数"颗粒度砍掉——庞大而简单的任务需要细粒度推进）
- [x] 子目标列表不出现滚动条（2026-08-24 验收修订）：更早已完成项折叠为"已完成 N 项"计数行，只展开最近一行（可撤销——撤销本就只能从末尾开始）；更换任务面板隐藏滚动条视觉（滚轮仍可用）
