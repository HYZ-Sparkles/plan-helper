...
- `src/router.ts` — hash 路由；窗口 label → 路由的映射在 `src/main.ts` 的 `routeByLabel`。
  ...
  - 窗口 label 常量：`pet` / `pet-menu` / `mini-board` / `main-board` / `control-panel`（与 `tauri.conf.json` 一致）。
...
- `scripts/gen-pet-frames.mjs` — **帧清单生成器**（资产更新可重跑）：从 `resourses/` 的 Oreo Cat sprite sheet 程序化切帧 → 生成 `src/lib/pet/animations.ts`。切分规则（对 sheet 像素实测）：帧间透明间隙 ≥4px = 帧边界；帧内容内部 ≤3px 断裂（抬爪与身体间的全透明列）吸附回同帧；>36px 的宽段 = 两帧粘连，在列覆盖低谷处分割。`--inspect DIR` 额外导出每行条带与逐帧裁剪 PNG 供人工核对。fps 默认值在脚本 `META` 表（资源无时序元数据，试拍后改——**用户已在验收中调过一轮，改动以 META 现值为准**）。**帧摆放微调**：`NUDGE` 表（动画号 → { 帧号 1-based: [ox, oy] 素材像素，正 = 右/下 }）只调摆放不动切分，验收时在调试页拨出草稿粘进来重跑生成即落盘。
- `src/lib/pet/animations.ts` — **生成物，勿手改**：`ANIMATIONS`（24 个动画的帧矩形 + fps + loop；键 = 作者标注编号）、`PET_SHEET_URL`（public/pet/oreo-sheet.png）、`SHEET_SCALE=2`、`SHEET_WIDTH/HEIGHT`。
- `src/lib/pet/engine.ts` — **PetEngine 动画引擎**（CONTEXT PetActionExecution 当前动作锁）：`request({steps, lock, onSettle})` 发起动作；steps = 顺序步（`{anim, loop?, fps?, flip?, movement?}`），末循环步开始/单次末步播完触发 `onSettle`（启动序列完成、模式过渡完成、再见退出都挂这里）；`stop()` 停在最后一帧（调试页用）；`subscribe` 订阅渲染状态。**锁的边界**：只在过渡期持有——末步 idle 循环开始（settled）即释放、任何新动作可替换稳态循环（否则启动序列的 7 循环会永久占锁）；过渡中用户动作拒绝一切、系统动作仅可被用户动作抢占（`state().locked` 同口径）。**步进续排**：`advanceStep` 非末步分支必须重排 rAF，否则多步序列第一步播完就停摆（启动序列曾卡死于 21 之后——两处均已修复并写进 Node 确定性回归）。**窗口位移**：`movement: {distance, direction}` 距离逻辑像素、`auto` = 朝屏幕余量大的一侧，经注入的 `PetMover` 应用、按工作区边界截短不出屏；位移插值用**单调步内时长**（`stepElapsed`），不能用帧内模累积 `acc`（每帧归零会锯齿——已修复的实际 bug）；末步完成时终点钉死 p=1。引擎纯 TS 无 DOM/Tauri 依赖，可 Node 直跑复现（工单 08 的锁/位移验证即这么做）。
- `src/lib/pet/menu.ts` — 桌宠菜单共享词汇：`PetMode` / `MenuAction` / `PetMenuState` 类型 + pet↔pet-menu 事件名常量（两窗口通信不各写一份魔法串）。
- `src/lib/pet/tauriMover.ts` — `createTauriMover(): PetMover` 的 Tauri 实现：物理坐标驱动桌宠窗口、钳制当前显示器工作区；JS 窗口 API 全异步而引擎同步取值 → 位置/工作区/尺寸/缩放启动时取一次缓存（跨显示器热切换刷新留给工单 09）。**09 的拖拽直接复用它**（moveTo 已带钳制）。
- `src/components/PetSprite.vue` — **桌宠帧渲染器**：单图 background-position 切帧；底部锚定（帧矩形含整行高，行底 = 地面线，蹲/坐/站/睡同高——用户验收硬约束）；每帧水平居中；整数倍缩放 + `image-rendering: pixelated`；`flip` 水平镜像（素材朝右，朝左移动用）。**帧摆放微调**：帧级 `ox/oy`（素材像素，来自清单或 `nudge` prop 实时覆写）叠加在锚定之上，translate 写在 scaleX 之前 → 偏移是屏幕空间（翻转不镜像偏移方向）。容器负责 flex 底对齐。
- `src/pages/DevAnimPage.vue` — **/dev/anim 动画调试页**（dev-only，浏览器直开 `http://localhost:1420/#/dev/anim`）：动画列表/播放/暂停/逐帧步进/调 fps（播放中实时生效）/循环/翻转/缩放（2×/4×/8× 检视切帧）/全帧平铺终审/位移模拟预览（模拟 mover 在 480×220 假想屏内滑动）。**帧摆放微调**：平铺格下方 ◀▲▼▶ 逐素材像素步进（↺ 清零），底部自动导出 NUDGE 表格式的草稿片段（帧号 1-based，一键复制）——粘进生成脚本重跑即落盘（草稿本身不持久化，同 fps 滑杆）。**帧边界、节奏、摆放问题都先来这里核对**。
- `src/windows/PetWindow.vue` — 桌宠窗口编排：启动序列 21→22→23→24→7（期间 pointer-events 关闭，点击/拖拽全禁用）→ `onSettle` 进正常态 + `shouldAutoOpenMainBoard` 检测（06 的启动触发接线）+ 700ms 站立节拍后系统动作 4→5 进工作睡眠；**启动后默认工作模式**；模式切换（菜单用户动作占锁）：休息→工作 4→5、工作→休息 6→7 + 隐藏小看板（67），切回工作恢复小看板（有有效当前任务才显示）+ auto-open 检测；休息模式站↔坐轮换（40s 一换，系统动作，09 的 PetRandomAction 上线后由 300s 权重抽取取代）；再见 = 17 播完 → `exitApp()`；系统关闭请求拦截（Alt+F4 无效）；点击 = 位移 <4px 的 pointerup，拖拽 = 手动 moveTo（带工作区钳制；完整形态——看板跟随/任务栏禁入/边缘吸附——归 09）；菜单失焦关闭与点击切换的竞态用 400ms 时间戳防抖。
- `src/windows/PetMenuWindow.vue` — 桌宠菜单独立小窗（桌宠窗口 64×64 装不下菜单）：三项 PetMenuActions（控制面板 PhSlidersHorizontal / 切换模式 PhCoffee↔PhBriefcase，按当前模式显示目标 / 再见 PhHandWaving）；播放用户动作期间项禁用 + tooltip「桌宠正在执行动作」（菜单可打开查看）；`tauri://blur` 失焦自隐藏；选择后 `emitTo("pet", "pet-menu:action")`。
